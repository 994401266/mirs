...
public class MenuController  extends BaseController {
	...
	public String list(HttpServletRequest request, Model model, Menu menu) throws Exception {
		ReturnDatas returnObject = null;
		if (SessionUser.getShiroUser().getAccount().equals("admin")) {
			returnObject = listalljson(request, model, menu);
		} else {
			returnObject = listjson(request, model, menu);
		}
	...
	public ReturnDatas listjson(HttpServletRequest request, Model model, Menu menu) throws Exception {
		...
		if (SessionUser.getShiroUser().getAccount().equals("admin")) {
			datas = menuService.findListDataByFinder(null, null, Menu.class, menu);
		} else {
			datas = userRoleMenuService.findMenuByUserIdAll(SessionUser.getUserId());
		}

...
var E3 = {
			...
			kingEditorParams : {
				...
				//指定上传文件请求的url。
				uploadJson : '/springrain/pic/upload',
			...
			// 格式化时长
			formatRuntime : function(val,row){
				if(val == null||val==''){
					return '';
				}
				return val+'min';
		        		...
		        		KindEditor.editor(E3.kingEditorParams).loadPlugin('multiimage',function(){
		        			...
		        			editor.plugin.multiImageDialog({
								clickFn : function(urlList) {
									...
									form.find("[name=coverLink]").val(imgArray.join(","));
									form.find("[name=pic]").val(imgArray.join(","));
									editor.hideDialog();
								}
							});
		        		});
		    		...
		    		_ele.unbind('click').click(function(){
		    			...
		    			.window({
		    			    ...
		    			    onOpen : function(){
		    			    	...
		    			    	$("ul",_win).tree({
		    			    		...
		    			    		onClick : function(node){
		    			    			if($(this).tree("isLeaf",node.target)){
		    			    				// 填写到cid中
		    			    				_ele.parent().find("[name=types]").val(node.text);
		    			    	...
		    			    	//$(_win).html("<div data-options='region:'south',border:false' style='text-align:right;padding:5px 0 0;'> <a class='easyui-linkbutton' data-options='iconCls:'icon-ok'' href='javascript:void(0)' onclick='javascript:alert('ok')' style='width:80px'>Ok</a></div>");
		    			    },
		    			    onClose : function(){
		    			    	$(this).window("destroy");
		    			    }
					...
					KindEditor.editor(E3.kingEditorParams).loadPlugin('image', function() {
						this.plugin.imageDialog({
							...
							clickFn : function(url, title, width, height, border, align) {
								...
								input.val(url);
								input.after("<a href='"+url+"' target='_blank'><img src='"+url+"' width='50' height='80'/></a>");
								this.hideDialog();
							}
						});
					});
